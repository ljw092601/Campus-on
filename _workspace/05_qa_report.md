...
## 라운드 4 — 3주차 기능 검증 (2026-07-14)

3주차 산출물(가이드 S5~S7·설정 S9·즐겨찾기 S10·6색 마커·오프라인 마감)을 정적 분석·자동 테스트·**Android 에뮬레이터 실기기**로 교차 검증.

### 정적/자동 검증
| 항목 | 결과 | 비고 |
|------|:---:|------|
| `flutter analyze` | 🟢 | No issues found (미사용 import 1건 발견→제거 후 재확인) |
| `flutter test` | 🟢 | 4건 통과 — `guide_flow_test`(S5→S6→S7 내비 / comingSoon 플레이스홀더 / 즐겨찾기 토글→S10 지속) + 스모크 |
| gen-l10n(ko/en) | 🟢 | 신규 키(가이드 섹션·설정 정보·즐겨찾기 세그먼트·카테고리 요약) 양 로케일 정합, 하드코딩 0 |

### 실기기 런타임 검증 (Pixel_7 / Android)
| 화면·기능 | 결과 | 확인 내용 |
|-----------|:---:|-----------|
| S5 가이드 카테고리 | 🟢 | 6종 색 아이콘·요약·항목수 배지(3/2/3/3/4/3, 목업 일치)·chevron |
| S6 항목 목록 | 🟢 | published 우선 정렬(외국인등록증 최상단), 즐겨찾기 별 |
| S7 행정 상세 | 🟢 | 4섹션 아코디언(개요·준비물 체크박스·단계 번호·관련링크/위치), 메타(예상 2–3주·난이도 ●●○), 외부 링크, 관련 위치 카드 |
| 딥링크(가이드→지도) | 🟢 | 위치 카드 탭→지도 탭 전환 + 해당 마커 Peek 자동 오픈 |
| 지도 6색 커스텀 마커 | 🟢 | 6 카테고리 티어드롭 핀이 각 색으로 렌더(오프라인 base64 에셋), 딥링크 setCenter |
| S9 설정 | 🟢 | 언어 라디오·즐겨찾기·정보 6항목(외부링크·라이선스·버전 0.2.0) |
| S10 즐겨찾기 | 🟢 | 시설/가이드 세그먼트, populated 목록, 스와이프 삭제 + 실행취소 스낵바, 빈 상태 CTA |
| i18n 한/영 | 🟢 | 기기 로케일 ko 렌더 정상, 가변 길이 레이아웃 무오버플로우 |

### 상태(5-state) 커버리지
- **Loading**: 가이드 목록/상세 스켈레톤(`SkeletonList`/`_DetailSkeleton`). **Empty**: S6 "준비 중" / S10 세그먼트별 빈 상태 + CTA. **Error**: 가이드 목록·상세 `ErrorStateView` 재시도. **Partial(핵심)**: comingSoon 항목 표준 플레이스홀더(+관련 위치 카드 유지). **Success**: 위 실기기 표.

### 오프라인 캐싱 마감 판정
- Firestore 영속성 명시(`persistenceEnabled`), 모든 읽기 경로 `Source.cache` 폴백(시설·가이드 getAll/getById/**getByCategory**). 즐겨찾기·언어·최근검색은 전면 로컬(SharedPreferences). ✅ 데이터 계층 오프라인 완료.
- 미포함(의도): 연결 상태 배너(connectivity) — 데이터 계층 무변경 Phase-2 후보로 문서화.
